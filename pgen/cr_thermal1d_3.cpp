...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
  ...
  for (int i=is; i<=ie; ++i) {
    ...
    for (int j=js; j<=je; ++j) {
      ...
      for (int k=ks; k<=ke; ++k) {
        ...
        phydro->u(IDN,k,j,i) += pcr->rho(ind)*0.1*exp(-SQR((x1 - 1.5*pcr->Hc)/0.1));
  ...
  if(MAGNETIC_FIELDS_ENABLED){
    ...
    for (int i=0; i<=(ie+1+NGHOST); ++i) {
      Real x1 = pcoord->x1f(i);
      int ind = 0;
      while ((ind < num_grid+1+2*NGHOST) && (xv[ind] < x1)) {ind += 1;}
      for (int j=js; j<=je; ++j) {
        for (int k=ks; k<=ke; ++k) {
          pfield->b.x1f(k,j,i) = pcr->bx(ind);
        }
      }
    }
     ...
     int ks, int ke, int ngh)
{
  ...
  auto G = [=](Real x) {
    Real x1 = x > 0.0 ? x : -x;
    Real g = pcr->g0*pow(x1/pcr->a, pcr->nu)/(pow(x1/pcr->a, pcr->nu) + 1.);
    g = x > 0.0 ? g : -g;
    return g;
  };
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=(NGHOST); ++i) {
        ...
        auto f = [=](Real r) {
          return (1. - 1./pcr->beta0)*alpha*pow(r, gc/(gc + 2.)) + (1. - zeta)*r - (1. + (1. - 1./pcr->beta0)*alpha + zeta);
        };
        ...
        Real va_face = b.x1f(k,j,is-i+1)/sqrt(rho_face);
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    for (int k=ks; k<=ke; ++k) { 
      for (int j=js; j<=je; ++j) { 
        for (int i=1; i<=(NGHOST); ++i) { 
          b.x1f(k,j,is-i) = pcr->bx(k,j,is-i); 
        } 
      }
    }
    for (int k=ks; k<=ke; ++k) {
     ...
     int ks, int ke, int ngh)
{
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=(NGHOST); ++i) {
        ...
        Real Lc_face = fabs(pc_face/dpcdx_face);
        Real va_face = b.x1f(k,j,ie+i)/sqrt(rho_face);
        Real thres = (dx/Lc_face)*(pcr->vmax/va_face);
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    for (int k=ks; k<=ke; ++k) { 
      for (int j=js; j<=je; ++j) { 
        for (int i=1; i<=NGHOST; ++i) { 
          b.x1f(k,j,ie+i+1) = pcr->bx(k,j,ie+i+1); 
        } 
      }
    }
    for (int k=ks; k<=ke; ++k) {
    ...
    for (int j=jl; j<=ju; ++j) {
      for (int k=kl; k<=ku; ++k) {
        ...
        if ((fabs(x1) >= pcr->x_min_cool_buffer_left) && (fabs(x1) <= pcr->x_max_cool_buffer_right)) {
          ...
          temp_new += (gg - 1.)*pcr->heat(ind)*dt/rho;
        } 
